...
const int EpollProxy::MAX_EVENTS = 4096;

EpollProxy::EpollProxy(const char *port, ZProcessor *processor) :
	...
	if (protocol == Const::PROTO_VAL_TCP) {
		...
		fprintf(stderr,
				"EpollProxy::EpollProxy(): <%s>, unrecognized IP family protocol\n",
				protocol.c_str());
		exit(1);
	}
}
...
int EpollProxy::create_and_bind(const char *port) {

	return create_and_bind(NULL, port);
}

int EpollProxy::create_and_bind(const char *host, const char *port) {
...
int EpollProxy::make_socket_non_blocking(const int& sfd) {
...
int EpollProxy::makeSvrSocket() { //only for svr
...
int EpollProxy::reuseSock(int sock) {
...
void* EpollProxy::threadedServe(void *arg) {

	EpollProxy *pes = (EpollProxy*) arg;
